...
RDEPEND="
	...
	dev-python/python-future[${PYTHON_USEDEP}]
...
src_prepare() {
	...
	for third_party_module in argparse bottle JediHTTP waitress requests; do
...
#	rm -r third_party/ycmd/{*.md,*.sh} || die
	...
	find third_party -name "*.py" -exec sed -i "1s/python3/python/" {} + || die
	egit_clean
